...
<?php
    ...
    $checkinErr = $checkoutErr = $tienAnErr = $websiteErr = "";
    $checkin = $checkout = $gender = $tienAn = $website = "";

    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (empty($_POST["tienAn"])) {
          $tienAnErr = "tienAn " . REQUIRED_MSG;
        } else {
          $tienAn = test_input($_POST["tienAn"]);
          if (!preg_match("/^[1-9]*$/",$tienAn)) {
            $tienAnErr = "only number"; 
          }
        }
    
        if (empty($_POST["checkbox"])) {
            $checkboxErr = "checkbox " . REQUIRED_MSG;
          } else {
            $gender = test_input($_POST["checkbox"]);
          }
        }
?>
